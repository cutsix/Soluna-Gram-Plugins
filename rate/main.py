""" Solgram currency exchange rates plugin. Plugin by @fruitymelon and @xtaodada"""
from json.decoder import JSONDecodeError

from solgram import log, logs
...
from solgram.services import client, scheduler
from solgram.utils import alias_command

DEFAULT_RATE = ["USDT", "CNY", "NGN", "TRY", "JPY", "EUR", "GBP"]
...
class Rate:
    ...
    async def get_rates(self, from_: str, tos_: [str], nb: float):
        logs.info(f"from: {from_} tos: {tos_} nb: {nb}")
        endpoint = f"{self.host}/currencies/{from_.lower()}.json"
        try:
            req = await client.get(endpoint, follow_redirects=True)
            rate__data = req.json()
            text = ""
            for to_ in tos_:
                text += f"`{from_} : {to_} = {nb} : {round(nb * rate__data[from_.lower()][to_.lower()], 4)}\n`"

            text += f'\n{self.lang_rate["warning"]} - {rate__data["date"]}'
            return text
        except Exception as e:
            logs.info(e, exc_info=True)
            return str(e)
...
async def hl(message: Message):
    ...
    if not rate_data.data:
        return
    try:
        rates = [parameter.upper().strip() for parameter in message.parameter]
        number = 1.0
        ...
        if rates[0] == "":
            rates = DEFAULT_RATE
        if len(rates) == 1:
            rates = rates + [x for x in DEFAULT_RATE if x not in rates]
        ...
        for to_ in tos_:
            if rate_data.currencies.count(to_) == 0:
                ...
                return
        text = await rate_data.get_rates(from_, tos_, number)
        await message.edit(text)
    except Exception as e:
        logs.info(e, exc_info=True)
        raise Exception(e)
