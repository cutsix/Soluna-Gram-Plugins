...
from solgram.utils import sleep, pip_install
...
**Note:** All trace settings only work in the current group.
"""
...
def get_chat_trace_key(chat_id: int, key_type: str, identifier: str = "") -> str:
    """Generate chat-specific trace key"""
    if identifier:
        return f"trace.{chat_id}.{key_type}.{identifier}"
    return f"trace.{chat_id}.{key_type}"
...
async def get_all_traced(client: Client, chat_id: int) -> Dict:
    """Get all traced users in current chat"""
    uid_reactions = {}
    chat_prefix = f"trace.{chat_id}.user_id."
    
    for key, value in cached_sqlite.items():
        if key.startswith(chat_prefix):
            user_id = int(key.split(chat_prefix)[1])
            uid_reactions[user_id] = {"reactions": value}
...
async def trace(bot: Client, message: Message):
    ...
    if len(message.parameter) == 0:  # Either untrace someone or throw error
        ...
        user_key = get_chat_trace_key(chat_id, "user_id", str(other_id))
        
        if not cached_sqlite.get(user_key, None):
            return await edit_and_delete(
                message, "This user is not traced in this group."
            )
        prev_emojis = cached_sqlite.get(user_key, None)

        del sqlite[user_key]
        del cached_sqlite[user_key]
        ...
        if message.parameter[0] in ["status", "clean"]:  # Get all traced info
            traced_uids = await get_all_traced(bot, chat_id)
            text, entities = (
                new_bold_string_entities("Traced userlist in this group:\n")
                if message.parameter[0] == "status"
                else new_bold_string_entities("Successfully untraced in this group: \n")
            )
            ...
            text, entities = append_bold_string(text, "\nTraced keywords in this group: \n", entities)
            keywordlist_key = get_chat_trace_key(chat_id, "keywordlist")
            if traced_keywords := cached_sqlite.get(keywordlist_key, None):
                for keyword in traced_keywords:
                    keyword_key = get_chat_trace_key(chat_id, "keyword", keyword.encode().hex())
                    reaction_list = cached_sqlite.get(keyword_key, None)
                    text += f"  {keyword}: "
            ...
            if message.parameter[0] == "clean":
                # Clean all trace data for this chat
                keys_to_delete = [k for k in cached_sqlite.keys() if k.startswith(f"trace.{chat_id}.")]
                for key in keys_to_delete:
                    del cached_sqlite[key]
                    del sqlite[key]
            return await edit_and_delete(
                message,
                text,
            ...
            # Reset all trace data for this chat
            keys_to_delete = [k for k in cached_sqlite.keys() if k.startswith(f"trace.{chat_id}.")]
            for key in keys_to_delete:
                del cached_sqlite[key]
                del sqlite[key]
            return await edit_and_delete(
                message,
                "**All trace data for this group has been reset.**",
                seconds=5,
                parse_mode=ParseMode.MARKDOWN,
            )
        else:
            if emojis := get_emojis_from_message(message):
                if not message.reply_to_message or not message.reply_to_message.from_user:
                    return await edit_and_delete(message, "Please reply to a message to trace a user.")
                    
                reaction_list = await gen_reaction_list(emojis, bot)
                if reaction_list:
                    user_key = get_chat_trace_key(chat_id, "user_id", str(message.reply_to_message.from_user.id))
                    sqlite[user_key] = reaction_list
                    cached_sqlite[user_key] = reaction_list
                    ...
                    text = "Successfully traced in this group: \n"
                    # TODO: Add username
            ...
            return await print_usage(message)
    elif len(message.parameter) == 2:  # log t|f; big t|f; kw del
        if message.parameter[0] == "log":
            ...
            keywordlist_key = get_chat_trace_key(chat_id, "keywordlist")
            keyword_key = get_chat_trace_key(chat_id, "keyword", keyword_encoded_hex)
            
            keywordlist = cached_sqlite.get(keywordlist_key, [])
            if keyword not in keywordlist:
                return await edit_and_delete(
                    message, f'Keyword "{keyword}" is not traced in this group.\n{keywordlist}'
                )
            if not cached_sqlite.get(keyword_key):
                return await edit_and_delete(
                    message, f'Keyword "{keyword}" is not traced in this group.'
                )

            prev_emojis = cached_sqlite.get(keyword_key)
            keywordlist.remove(keyword)
            sqlite[keywordlist_key] = keywordlist
            cached_sqlite[keywordlist_key] = keywordlist
            del sqlite[keyword_key]
            del cached_sqlite[keyword_key]

            text, entities = new_bold_string_entities(
                "Successfully untraced keyword in this group: \n"
            )
            text += f"  {keyword}: "
            text, entities = append_emoji_to_text(text, prev_emojis, entities)
            return await edit_and_delete(
        ...
        if keyword and len(emojis) != 0:
            ...
            if reaction_list:
                ...
                sqlite[keyword_key] = reaction_list
                cached_sqlite[keyword_key] = reaction_list

                if cached_sqlite.get(keywordlist_key, None) is None:
                    sqlite[keywordlist_key] = [keyword]
                    cached_sqlite[keywordlist_key] = [keyword]
                elif keyword not in cached_sqlite[keywordlist_key]:
                    cached_sqlite[keywordlist_key].append(keyword)
                    sqlite[keywordlist_key] = cached_sqlite[keywordlist_key]

                text, entities = new_bold_string_entities(
                    "Successfully traced keyword in this group: \n"
                )
                text += f"  {keyword}: "
                text, entities = append_emoji_to_text(text, reaction_list, entities)
                return await edit_and_delete(
...
async def trace_user(client: Client, message: Message):
    ...
    chat_id = message.chat.id
    user_key = get_chat_trace_key(chat_id, "user_id", str(message.from_user.id))
    
    with contextlib.suppress(Exception):
        if reaction_list := cached_sqlite.get(user_key, None):
            await client.invoke(
                SendReaction(
...
async def trace_keyword(client: Client, message: Message):
    ...
    with contextlib.suppress(Exception):
        if message.text:
            if keyword_list := cached_sqlite.get(keywordlist_key, None):
                for keyword in keyword_list:
                    if keyword in message.text:
                        keyword_key = get_chat_trace_key(chat_id, "keyword", keyword.encode().hex())
                        if reaction_list := cached_sqlite.get(keyword_key, None):
                            await client.invoke(
                                SendReaction(
